...
export default function Navbar() {
    ...
    useEffect(() => {
        ...
        // Proteksi Akun: Hanya auto-logout jika terjadi rotasi password admin (forceLogout)
        // Dihapus untuk isSuspended karena memicu "client-side redirect loop" pada beberapa browser
        if (session && !isLoggingOut) {
            const forceLogout = (session as any).forceLogout;

            if (forceLogout) {
                setIsLoggingOut(true);
                console.log("[AUTH] Session expired due to password rotation. Logging out...");
    ...
    return (
        ...
        >
            {/* Maintenance Banner — full width di atas navbar */}
            <MaintenanceBanner />
