    ...
    return (
        <section className="flex flex-col gap-12 w-full">
            ...
            <div className="flex flex-col gap-8 items-center max-w-5xl mx-auto w-full px-4">
                ...
                <div className="w-full flex flex-col gap-6 bg-[#121214] p-5 md:p-8 rounded-[2.5rem] border border-white/5 shadow-2xl relative overflow-hidden group">
                            ...
                            <button
                                ...
                                className={`flex-shrink-0 min-w-[38px] h-9 flex items-center justify-center rounded-xl text-[10px] font-black transition-all duration-300 border ${
                                    letter === l
                                        ...
                                        : "bg-white/5 text-zinc-600 hover:bg-white/10 hover:text-white border-white/5"
                                }`}
                            >
                                {l}
                            </button>
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full bg-black/40 border border-white/5 rounded-2xl pl-12 pr-5 py-4 text-xs text-white placeholder:text-zinc-800 outline-none focus:border-primary/40 transition-all font-bold"
                        />
                    </div>
                </div>
            </div>
            ...
            {letter === "All" && search === "" && isClient && (
                <div className="max-w-7xl mx-auto w-full px-6 flex flex-col gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-[10px] md:text-xs font-black text-zinc-500 uppercase tracking-widest">Trending Categories</h2>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {featuredTags.map((tag) => (
                            <Link
                                ...
                                className="relative h-16 md:h-24 flex items-center justify-center bg-zinc-900 border border-white/5 rounded-2xl hover:border-primary/30 hover:bg-primary/5 transition-all group overflow-hidden"
                            >
                                <span className="relative z-10 text-[10px] md:text-xs font-black text-zinc-500 group-hover:text-primary uppercase tracking-widest">{tag}</span>
                            </Link>
                        ))}
                    </div>
                </div>
            ...
            <div className="max-w-7xl mx-auto w-full px-6 flex flex-col gap-8 mb-24">
                {isClient && Object.keys(groupedTags).sort().map(char => (
                    <div key={char} className="flex flex-col md:flex-row gap-4 md:gap-8">
                        <div className="md:w-20 shrink-0">
                            <div className="sticky top-28 flex items-baseline gap-2">
                                <span className="text-4xl font-black text-primary/10 leading-none">{char}</span>
                        ...
                        <div className="flex-1 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
                            {groupedTags[char].map(tag => (
                                <Link
                                    key={tag}
                                    href={`/search?q=${encodeURIComponent(tag)}`}
                                    className="px-4 py-3 bg-white/[0.02] border border-white/5 rounded-xl text-[10px] font-bold text-zinc-600 hover:text-primary hover:bg-primary/5 hover:border-primary/20 transition-all uppercase tracking-wider truncate"
                                >
                                    {tag}
                                </Link>
                            ))}
